...
class CustomChatCompletionMessageToolCallParam(TypedDict, total=False):
    id: Required[str]
    type: Required[Literal["function"]]
    function: Required[ToolCallFunctionParam]
    predicted_tool_round_trip_seconds: Annotated[
        float, Field(ge=0, allow_inf_nan=False)
    ]
    """The model's predicted round-trip time for this tool call, in seconds."""
...
class CustomChatCompletionMessageParam(TypedDict, total=False):
    ...
    tool_calls: list[CustomChatCompletionMessageToolCallParam] | None
...
class ConversationMessage(TypedDict, total=False):
    ...
    observed_tool_round_trip_seconds: float
    """Server-observed round-trip time of the tool call, in seconds."""
...
def _parse_chat_message_content(
    ...
    for result_msg in result:
        if role == "assistant":
            ...
            # Populated internally after public request validation.
            observed_round_trip = cast(dict[str, Any], message).get(
                "observed_tool_round_trip_seconds"
            )
            if observed_round_trip is not None:
                result_msg["observed_tool_round_trip_seconds"] = observed_round_trip
